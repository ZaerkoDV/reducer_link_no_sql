...
public final class ObjUtils {
    ...
    public static ObjectId asObjectId(Object node, String key) {
        return asObjectId(node, key, new ObjectId());
    }

    public static ObjectId asObjectId(Object node, String key, ObjectId def) {
        Object value;

        if ((node instanceof Map) && ((value = ((Map)node).get(key)) != null)) {
            if (value instanceof Number) {
                return new ObjectId(value.toString()); //((Number)value).longValue();
            } else {
                try {
                    return new ObjectId(value.toString()); //Long.valueOf(String.valueOf(value));
                } catch (NumberFormatException ex) {
                    return def;
                }
            }
